...
class Glat(FairseqNATModel):
    ...
    @classmethod
    def build_decoder(cls, args, tgt_dict, tgt_type_dict, embed_tokens):
        decoder = NATransformerDecoder(args, tgt_dict, tgt_type_dict, embed_tokens)
        if getattr(args, "apply_bert_init", False):
            decoder.apply(init_bert_params)
        return decoder

    def forward(
            self, src_tokens, src_lengths, prev_output_tokens, tgt_tokens, tgt_types, glat=None, **kwargs
    ):
        # encoding
        ...
        keyword_positions = tgt_types.eq(self.keyword).type(torch.int64)
        operator_positions = tgt_types.eq(self.operator).type(torch.int64)
        name_positions = tgt_types.eq(self.var).type(torch.int64)
            ...
            with torch.no_grad():
                ...
                for li in range(bsz):
                    ...
                    if target_num > 0:
                        ...
                        # random sampling for 70% of the time
                        if random.uniform(0, 1) > 0.3:
                            input_mask[li].scatter_(dim=0, index=torch.randperm(seq_lens[li])[:target_num].cuda(), value=0)
                        # choosed-type first learning
                        else:
