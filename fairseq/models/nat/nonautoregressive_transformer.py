...
class NATransformerDecoder(FairseqNATDecoder):
    def __init__(self, args, dictionary, type_dictionary, embed_tokens, no_encoder_attn=False):
        super().__init__(
        ...
        self.keyword = type_dictionary.keyword()
        self.operator = type_dictionary.operator()
        self.var = type_dictionary.variable()
    ...
    def extract_features(
        ...
        **unused
        ...
        if embedding_copy:
            ...
            # attn_score_emb = torch.bmm(self.copy_attn(positions),
            #                     src_embd.transpose(1, 2))
            # attn_score_pos = torch.bmm(self.copy_attn(positions),
            #                        encoder_out['encoder_pos'][0].transpose(1, 2))
                                   
            if src_mask is not None:
                attn_score = attn_score.masked_fill(src_mask.unsqueeze(1).expand(-1, seq_len, -1), float('-inf'))
                # attn_score_emb = attn_score_emb.masked_fill(src_mask.unsqueeze(1).expand(-1, seq_len, -1), float('-inf'))
                # attn_score_pos = attn_score_pos.masked_fill(src_mask.unsqueeze(1).expand(-1, seq_len, -1), float('-inf'))

            attn_weight = F.softmax(attn_score, dim=-1)
            # attn_weight_emb = F.softmax(attn_score_emb, dim=-1)
            # attn_weight_pos = F.softmax(attn_score_pos, dim=-1)
           
            x = torch.bmm(attn_weight, src_embd)
            # x = 0.7*torch.bmm(attn_weight_emb, src_embd) + 0.3*torch.bmm(attn_weight_pos, src_embd)
